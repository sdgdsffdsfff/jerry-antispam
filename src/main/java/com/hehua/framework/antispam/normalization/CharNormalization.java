...
public class CharNormalization {
     ...
     * @param convertSynonymy 过滤同义字符
     * @param filterSymbol 是否过滤symbol字符（包括"\n""\r"" ""\t"等,见Symbol_CT.txt），
     * @param keepLastSymbol 连续symbol字符是否保留一个
     * @return 转换后的字符串，长度可能比转换前的短
     */
    public static String compositeTextConvert(String src, boolean needT2S, boolean needM2S,
            boolean needDBC, boolean ignoreCase, boolean filterNoneHanLetter,
            boolean convertSynonymy, boolean filterSymbol, boolean keepLastSymbol) {
        ...
        for (int i = 0; i < chs.length; i++) {
            char c = compositeCharConvert(chs[i], needT2S, needM2S, needDBC, ignoreCase,
     ...
     * @return 转换后的字符串，长度可能比转换前的短
     */
    public static final char compositeCharConvert(char c, boolean needT2S, boolean needM2S,
            boolean needDBC, boolean ignoreCase, boolean filterNoneHanLetter,
