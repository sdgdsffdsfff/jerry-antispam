...
public class ExtractInfomation {
	...
	// "'" , "(" , ")" , ",","{" , "}" , "," , "\\", "^","~" , "[" , "]","`",
	// "<" , ">" , "#" , "%" ,";" , "/" , "?" , ":" , "@" , "&" , "="};
	// private static HashMap<String,Integer> URLCharMap = new
	...
	public static ExtractInfomation getInstance() {
		return instance;
	}
	...
	public String extractEntity(String text) {
		...
		for (int i = 0; i < array.length; i++) { // 遍历字符串
			// 在这个范围内的是汉字、数字和英文
			if ((array[i] >= 19968 && array[i] <= 40869 && array[i] != '丨')
					|| (array[i] >= '0' && array[i] <= '9')
					|| (array[i] >= 'a' && array[i] <= 'z')
					|| (array[i] >= 'A' && array[i] <= 'Z'))
	...
	/**
	 * 提取文本中以分隔符隔开的url,分隔符包括汉字和 public ArrayList<String>
	 * extractURLContent(String text) { if (text == null) return null;
	 * ArrayList<String> urlContentList = new ArrayList<String>(); StringBuffer
	 * buff = new StringBuffer(); char[] array = text.toCharArray(); for (int i
	 * = 0; i < array.length; i++) { // 遍历字符串 if ((array[i] >= 19968 && array[i]
	 * <= 40869)) buff.append(array[i]); // 是汉字就加入 } return buff.toString(); }
	 */
	/**
	 * 从右到左从上到下的竖排文字解析
	 * 
